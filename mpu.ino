...
//  if(passThru)MahonyQuaternionUpdate(-ax, ay, az, gx*PI/180.0f, -gy*PI/180.0f, -gz*PI/180.0f,  my,  -mx, mz);
    ...
    if (delt_t > 17) {
      ...
      roll  = atan2f(a31, a33);
      yaw   = atan2f(a12, a22);
      yaw   += 3.51 * M_PI/180; // Declination at Aalborg, Denmark is +3.51 degrees on 2020-11-26
      if(yaw < 0) yaw += 2*M_PI; // Ensure yaw stays between 0 and 2 pi
      lin_ax = ax + a31;
      lin_ay = ay + a32;
      lin_az = az - a33;
      Serial.print(yaw, 2);
      ...
      buf[14]=0;
      buf[15]=0;
      port.beginPacket(ip,remotePort);              
      port.write(buf, 16);
      port.endPacket();
